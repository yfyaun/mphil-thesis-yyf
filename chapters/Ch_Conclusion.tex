...
observations are organized through a bird's-eye-view (BEV) representation so
...
their per-beam power distributions. The resulting Top-K candidate beams provide
a sensing-driven interface between the perception system and the subsequent
...
evaluation methodology. The planned evaluation connects road traffic
...
together. This evaluation pipeline is intended to make explicit how errors in
sensing and identity maintenance may propagate into communication-level
performance.

Because the experimental results are not yet available, this conclusion does
not claim demonstrated throughput improvement, beam prediction superiority, or
robustness under specific operating conditions. TODO[needs-evidence]: After
Chapter~\ref{ch:experiments} is finalized, summarize only the findings directly
supported by the completed experiments, including the relevant baselines,
metrics, figures, and statistical evidence.

\section{Limitations}
\label{sec:conclusion-limitations}

Several limitations follow from the current scope of the thesis. First, the
evaluation is expected to rely on a simulated or generated multimodal
...
traffic behavior, occlusion patterns, and propagation conditions in real
...
control signaling, HARQ behavior, scheduler interaction, or mobility management
...
robustness limits and failure thresholds still require experimental study.
...
TODO[needs-evidence]: After the ablation and robustness experiments in
Chapter~\ref{ch:experiments} are completed, refine this section so that each
limitation is tied to the observed failure cases, sensitivity trends, or
...
protocol-level simulation. Future work may incorporate more detailed modeling
of SSB and CSI-RS timing, beam reporting, feedback delay, scheduling, control
overhead, retransmission behavior, and mobility procedures. This would allow
...
\subsection{Robustness to Missing or Degraded Modalities}
\label{subsec:future-modality-robustness}

Another important direction is robustness under missing, degraded, or
conflicting sensing modalities. In realistic deployments, some cameras may be
blocked, calibration may drift, weather and illumination may reduce visual
quality, and ISAC point clouds may become sparse or noisy. Future models could
therefore include confidence-aware multimodal fusion, modality dropout during
training, explicit uncertainty estimation, and fallback strategies that
increase communication measurements when sensing reliability decreases.
...
incorrect associations or unstable beam predictions during deployment.
