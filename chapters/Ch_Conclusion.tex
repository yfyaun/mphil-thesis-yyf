...
serving BS and nearby SNs provide multiview
camera observations, and the BS also provides an ISAC point cloud. These
observations are organised through a BEV representation so
that vehicle detection and per-target beam prediction share a common spatial
frame.

The first part of the work formulated the Distributed Multimodal
Sensing-Assisted Beam Management framework, denoted by DMSA-BM. In this
framework, multiview camera features and
...
the frequency and scope of expensive beam measurements when the target-to-user
association is accepted and its track is currently observed.
...
measurements establish an initial target-to-user association. An IMM tracker
subsequently maintains the temporal continuity of
...
Under the common experimental configuration, distributed multimodal attention
achieved the highest overall AP@2m, Recall@2m, and Top-4 power ratio among the
evaluated perception variants, while no single fusion method dominated every
beam-prediction metric. At the communication level, DMSA-BM achieved a
three-condition macro-average effective user rate of \(146.332\) Mbps with
\(1.56\%\) CSI-RS overhead. The corresponding SSB-guided refinement results
were \(142.248\) Mbps with \(1.90\%\) overhead for
\(K_{\mathrm{ref}}=4\), and \(115.890\) Mbps with \(5.71\%\) overhead for
\(K_{\mathrm{ref}}=12\). The tracker variants produced comparatively close
end-to-end results. These findings support the use of distributed multimodal
sensing for conditional candidate reduction under the stated operating point,
without implying that sensing use alone or a particular tracker guarantees a
communication gain.
...
protocol-level details such as physical-resource mapping, feedback latency,
control signalling, hybrid automatic repeat request (HARQ) behaviour, or
mobility-management procedures. As a
result, throughput-related quantities should be interpreted as
...
target-to-user association, and tracking. Association rejection, conventional
...
of SSB and CSI-RS timing, beam reporting, feedback delay, control overhead,
retransmission behaviour, and mobility procedures. This would allow
