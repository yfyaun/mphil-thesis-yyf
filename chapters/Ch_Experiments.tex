...
power, noise model, RZF processing, and resource
accounting.  The resulting communication model is a 5G NR-oriented abstraction
at the beam-measurement level rather than a physical resource block (PRB)- and
resource element (RE)-level protocol model.
...
Target detection is evaluated by average precision (AP)@2m and Recall@2m.  The distance qualifier
...
from measured CSI-RS candidates.  The tracker comparison applies IMM, a
CV Kalman-filter baseline (KF-CV), and a CT Kalman-filter baseline (KF-CT) to
the same Top-1 CE detector and communication procedure.
...
\(145.880\) Mbps for KF-CT.  KF-CV is marginally higher than IMM at this
operating point, but the limited separation among the three variants does not
establish universal superiority across motion conditions or system
configurations.
