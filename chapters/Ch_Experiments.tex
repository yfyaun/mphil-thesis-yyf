...
determine whether a sensing-derived candidate set is used; and the resulting measurement
policy is assessed through average effective user rate and CSI-RS overhead.  The
...
same codebooks, RZF processing, and resource configuration.  The
perception-assisted policy scans a sensing-derived candidate set only when the
association is accepted and its track is currently observed; otherwise, it
executes conventional refinement.  The tracker
...
\section{Association-Gated Beam Management}
...
\(69.93\%\).  Thus, the policy retains conventional refinement whenever no
...
and Night is therefore an integral part of the association-gated operating
policy and should be interpreted together with the average-rate result.
