...
the algorithmic data flow and system interface, while the quantitative
performance evaluation is presented in Chapter~\ref{ch:experiments}.
...
The fusion design assumes that temporal synchronization and spatial calibration
are handled before BEV fusion. In practice, calibration and synchronization
errors can affect both detection and beam prediction. Therefore, the framework
keeps explicit sensor timestamps, camera extrinsics, point-cloud coordinate
transforms, and missing-modality indicators as part of the sample metadata.
...
The training objective combines detection and beam-prediction terms,
\begin{equation}
  \mathcal{L}
  =
...
terms. The detection term follows the CenterNet-style combination of heatmap
classification and target-attribute regression, while the beam term compares
the predicted beam distribution with the beam labels generated from the channel
...
\end{equation}
The spectra are normalized before matching, and the one-to-one assignment is
solved with a bipartite matching procedure. Matches whose costs exceed a
reliability threshold are rejected and handled by later re-association or
partial-measurement recovery.
...
age limits. The IMM model set includes constant-velocity, constant-acceleration,
and turning-motion hypotheses, with gating and track-management rules used to
control prediction-only maintenance and re-association.
...
These equations define an evaluation interface rather than a complete
communication protocol. Beam mismatch is represented through the effective
channel used by the ZF abstraction, while the overhead term accounts for full
beam sweep, partial measurement, calibration, and re-association resources.
...
the association cost matrix is formed, this step scales cubically in the larger
side of the matching problem. Fourth, IMM tracking scales with the number of active
...
This complexity discussion identifies the dominant algorithmic terms. Measured
runtime and memory usage can be reported together with the experimental setup
when implementation-specific profiling is available.
