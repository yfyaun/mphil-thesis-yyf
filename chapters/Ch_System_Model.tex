...
observations, the target-user association variables, the tracking states, and the
communication model used for beam management and system-level evaluation. The
model supports the sensing-assisted beam management framework developed in the
following chapter and keeps implementation-specific parameters explicit.
...
number of cooperating SNs is denoted by \(K_{\mathrm{SN}}=|\mathcal{S}_b|\).
Each SN contributes sensing observations but does
...
coarser beam-management interval according to the evaluation granularity. At
...
whereas subsequent epochs rely more heavily on sensing-assisted
...
anchored to the BS or to a fixed road-side reference frame. The simulation
layout, the value of \(K_{\mathrm{SN}}\), the frame duration, and the spatial
extent of the BEV region are explicit scenario parameters.
...
point cloud is used as an ISAC-like geometric observation and should not be
interpreted as a fully specified radar signal-processing model.
...
operations are specified through the BEV fusion functions in the method chapter.
...
are included when they support association and tracking.
...
maintains the estimated motion of a physical vehicle across epochs. Defining
these variables before the communication model makes explicit that beam
measurements are user-indexed, whereas sensing predictions are target-indexed.
...
\(C^{\mathrm{init}}_{b,i,u}(t_0)\) is an initial matching cost that combines
beam-spectrum similarity, BEV-domain consistency, and detection confidence.
...
vehicles, the tracker uses an interacting multiple model (IMM) filter with a
set of motion models \(\mathcal{Q}\), such as constant-velocity,
constant-acceleration, and turning-motion models. The probability of motion model
...
association function based on spatial innovation, motion consistency, detection
confidence, and optionally beam-distribution consistency. After a successful track-to-detection association, the
track state, the target-user mapping, and the beam prediction for the
corresponding communication user are updated jointly.

\section{Communication and Beam Management Model}
\label{sec:system-communication-model}

This section combines the codebook-level beam model, communication measurements,
beam mismatch abstraction, and system-level performance metrics. The goal is to
define a communication model that is detailed enough to connect sensing-assisted
beam decisions to throughput, while remaining simpler than a full 5G NR
...
\end{equation}
This communication model allows the evaluation to account for two competing
effects: sensing-assisted prediction can reduce beam training overhead, while
prediction, association, or tracking failures can reduce the SINR through beam
mismatch or imperfect effective-channel estimation.

The codebook size, beam-label generation procedure, beam-power normalization,
partial-measurement abstraction, mismatch factor, and overhead unit are treated
as evaluation parameters shared across the proposed method and baselines.
...
\label{sec:system-problem-formulation}
...
this objective is treated as a coupled system-design and evaluation problem
...
\(R^{\mathrm{adj}}_{b,u}(t)\) under the communication model in
Section~\ref{sec:system-communication-model}. The learning objectives, matching
costs, overhead accounting, and mismatch penalties are defined consistently in
the method and evaluation chapters.
