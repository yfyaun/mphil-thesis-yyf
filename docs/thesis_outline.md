...
- 以 Table 5.1 总体比较六类感知/融合方案；按天气的结果拆分为 Table 5.2 的 detection metrics 和 Table 5.3 的 beam-prediction metrics，以避免将不同层次指标压缩进过宽表格。叙述应先指出不同指标并不存在统一最优方案：cross-agent multimodal 的 AP@2m 为 78.73%，mean-gated multimodal 的 Recall@2m 和 Top-4 power ratio 分别为 81.90% 和 97.13%，而 single-station multimodal 的 Beam Top-1@2m 为 71.54%。
