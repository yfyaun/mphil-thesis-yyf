...
- 以 target BS 为单位定义从 \(\mathcal O_{b,t}\) 到未绑定 detections、quality distributions、perception
  tracks、accepted UE--track bindings 及 CSI-RS candidates/fallback 的数据流。
- 说明 physical target、perception track 和 communication UE 的三层角色：感知网络不接收或输出 UE
  identity；通信 measurement 只在 association event 进入；final service beam 永远来自实际 CSI-RS
  measurement。
- 设置 overview figure，明确 sensing evidence、track state、communication measurement 和 resource-aware
  candidate decision 的信息边界；GT/oracle 仅连接诊断和评估支路。
- 以简洁映射总结方法：
  \(\mathcal O_{b,t}\rightarrow\{d_{i,t},\mathbf p_{i,t}\}\rightarrow\mathcal R_{b,t}
  \rightarrow\pi_{b,t}\rightarrow\mathcal C_{b,u,t}\)。

### 4.2 Distributed Multimodal BEV Representation

写作要点：

- 定义 \(\mathcal J_b=\{b\}\cup\mathcal N_b\)，将 target-BS/SN image features 经标定投影或 lift 至
  target-BS-centered BEV，并将 BS-local ISAC point cloud 编码到同一平面。
...
- 说明共享 BEV 的设计动机：SN 提供遮挡/视场互补，target BS 保持与 serving link 的几何关系，ISAC 提供
  与视觉互补的局部几何证据。
- 用 availability/confidence mask 处理 missing、noisy 或不同覆盖条件的输入；不把同步和标定误差的具体
  工程补偿写成本章的主要贡献。

### 4.3 Joint Detection and Beam-Quality Learning
...
#### 4.3.2 Distributional Beam Supervision
...
### 4.4 Target-to-User Association and Track Maintenance

写作要点：

- 先说明 tracking 维持的是 perception target identity，association 将该 target-level state 连接到
  communication UE；二者不可相互替代。
- 以 partial mapping \(\pi_{b,t}:\mathcal U_b[t]\rightarrow\mathcal R_{b,t}\cup\{\varnothing\}\) 表示
  accepted UE--track bindings，并保留未匹配状态作为保守决策的一部分。

#### 4.4.1 Track Maintenance

- 对每个 split、weather 和 target BS 独立维护 tracks；以 CV KF、CT EKF 与 IMM(CV+CT) 作为可比较
  motion estimators。给出最小 kinematic state
  \(\mathbf x_t=[p_x,p_y,v_x,v_y]^{\mathsf T}\) 及 detection measurement
  \(\mathbf z_t=[p_x,p_y]^{\mathsf T}\)。
- 用 gated Hungarian assignment 将 current detections 与 predicted tracks 关联；spatial innovation、
  motion consistency、detection confidence 与可选 beam-distribution consistency 可构成 track association
  cost。
- track update 同时刷新 target-level distribution、confidence 与 hint age；失配或未观测不生成正式
  beam candidate，而是使 association 进入 recovery/reassociation 的条件。

#### 4.4.2 Measurement-Assisted Association

- 在 initial access、new serving cell、binding failure 或 sensing evidence 不可靠时执行 conventional
  SSB/CSI-RS measurement。令实测 CSI beam 为 \(b_u\)，以
  \(c(u,i)=-\log p_i(b_u)/\log M_{\mathrm{CSI}}\) 构造 UE--track beam-likelihood cost。
- 以 CSI-to-SSB parent map 检查可行 pair；Hungarian assignment 在可行 pair 和 dummy unmatched columns
  上求解一对一映射。高 cost、低 margin 或低 reliability 的 pair 被拒绝而非强制 binding。
- 强调该步骤不使用 GT、几何反推 beam 或感知直接识别 UE；beam direction 与 beam top-\(K\) 只作为
  association ablations。

### 4.5 Reliability-Gated Candidate Selection

写作要点：

- 定义可靠度 gate \(G_{u,t}\)，其证据包括 binding confidence、assignment margin、current detection/track
  confidence、hint age、SSB-parent consistency、observation count 与 beam-score mass，且均不得依赖 GT。
...
- 给出逐 sensing epoch 的伪代码：BEV representation、joint inference、track prediction/update、event-driven
  association/reassociation、reliability test、candidate/fallback decision 与 measured-beam interface。
