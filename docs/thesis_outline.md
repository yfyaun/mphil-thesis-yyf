...
显式处理。随后给出以 target BS 为中心的研究目标、范围和四项贡献，并用章节安排说明每一项
主张将如何由后续方法和实验支撑。
...
- 明确总体目标：构建一个以 target BS 为局部协调点的 multimodal sensing-assisted beam-alignment
  framework，将多模态、跨节点环境感知转化为服务正确 UE 的低开销 CSI-RS candidate selection，
  而不绕过实际通信测量。
- 目标一：在统一局部 BEV 表征中融合 target-BS/SN 相机与 BS-local ISAC 点云，联合输出车辆
  detection 和 192-way CSI beam-quality distribution，以检验跨模态和跨节点信息是否在不同
  可见性、遮挡和天气条件下提供互补收益。
- 目标二：把 sensing target-to-user association 形式化为通信测量辅助的一对一 correspondence
  estimation。初始接入或重关联阶段以 conventional SSB/CSI-RS 结果和感知目标的 beam likelihood
  建立映射；多目标状态估计与 data association 用于维持该 correspondence 的时间一致性。
- 目标三：只对具有可靠 correspondence 且当前被观测到的 target 使用 prediction-derived candidates；
  UE 在实际测量的 CSI-RS candidates 中选择 beam，不可靠时保持 conventional refinement。
- 目标四：以分层证据链连接 perception、beam ranking、target-to-user association、correspondence
  maintenance、candidate selection 与 RZF/resource-aware communication evaluation；不以单帧 Top-\(K\)
  accuracy 代替通信系统层结论。
- 说明范围：局部 target BS 内的 association 与 correspondence maintenance，不主张跨 BS global
  identity；区分 \(K_{\mathrm{model}}\)、\(K_{\mathrm{bind}}\) 和 \(K_{\mathrm{scan}}\)；GT 只用于
  诊断或上界，主结果采用冻结 protocol 下的非 GT 决策路径。

### 1.4 Contributions

写作要点：

- 贡献 1：构建 target-BS-centered BEV 的多模态、跨感知节点联合 detection--192-way CSI beam
  quality prediction，使 object-level perception 与 link-level supervision 在同一空间网格中对齐，
  并针对感知条件和节点覆盖差异评价互补性。
- 贡献 2：采用 CSI power distribution + ranking 的 beam supervision，令输出同时服务于候选 beam
  排序、置信度刻画和 association likelihood；与 strongest-beam classification 和 KL-only 的受控
  对比检验候选价值。
- 贡献 3：提出面向 identity-consistent beam alignment 的 sensing target-to-user association 框架：
  未绑定感知输出经多目标轨迹维护后，利用传统测量完成 initial/re-association，并通过 SSB-parent
  consistency、reliability gate 与 conventional fallback 保证 beam hint 只用于可信 correspondence。
- 贡献 4：建立由 detection、beam distribution、target-to-user association、correspondence
  maintenance、candidate recall、pilot overhead、RZF effective rate、p05 rate 和 outage 构成的
  分层评价方法，检验感知辅助对实际 beam alignment 的系统影响。
...
- Chapter 4 介绍跨节点 BEV fusion、joint detection--beam distribution learning、多目标状态估计、
  measurement-assisted target-to-user association、reliability-gated candidate selection 与通信接口。
- Chapter 5 描述既有数据集上的实验协议、条件分层对照、association/correspondence 指标和系统级
  evaluation；所有比较在一致的 measurement/resource profile 下进行。
...
- 指出仅报告 Top-\(K\) accuracy 的不足，并引出从 target-to-user association 到候选扫描、资源记账
  和 RZF effective-rate 的系统级证据链。gap 与 Chapter 1 的四项贡献对齐。
...
## Chapter 5: Dataset Protocol and Experimental Evaluation

本章目的：在既有 `mmbeam_town05_3weather_v1` 数据集和冻结 radio profile 下，建立从条件化
多模态感知到 identity-consistent beam alignment 的评价协议。重点不是重新描述数据资产的生成，
而是说明样本、坐标、码本、天气、切分、baseline、resource accounting 和分层指标如何保证公平比较。
...
### 5.1 Dataset, Scenario, and Frozen Evaluation Protocol
...
叙述逻辑：先回到研究问题，再概括四项贡献对应的完成内容，随后总结由实验支持的结论，
...
- 概括 measurement-assisted sensing target-to-user association 与 correspondence maintenance 如何使
  beam hint 服务于正确 UE。
- 概括可靠度门控候选扫描及其由 perception 到 resource-aware effective rate 的系统级评价方法。
