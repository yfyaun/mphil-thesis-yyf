...
- Chapter 3 定义 target-BS local coordinate、观测、48/192 codebook、physical target/perception track/
  communication UE 三类身份、target-to-user association state、candidate set 和 resource accounting。
...
target-BS-centered multimodal observations、sensing target-to-user association、candidate
measurement 与资源记账模型，为 identity-consistent beam alignment 提供统一符号和边界。

叙述逻辑：先定义局部网络、时间尺度和坐标系，再定义多模态感知观测与未绑定 detection，随后
形式化三类身份及其 association，最后定义 48/192 codebook、candidate measurement、reliability
...
### 3.3 Sensing Target-to-User Association State

写作要点：

- 区分 physical target、perception-track 和 communication UE 三类身份；定义 UE--track association
...
- 定义 initial/re-association 的 conventional SSB/CSI-RS measurement，以及可靠 association 下由
...
- 用自然语言和必要符号总结本文问题：给定 target-BS-centered multimodal observations 和周期性
  ...
  sensing target-to-user association 和可靠度门控 CSI-RS candidates。
- 采用多目标表述：提高条件 beam-ranking quality、target-to-user association accuracy 与
  association persistence，在保持 candidate recall、p05 rate 和 outage 可接受的前提下降低 CSI-RS
  resource overhead。
