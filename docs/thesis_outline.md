...
- **挑战三：association-aware candidate reduction 与系统评价。** 感知不应无条件取代通信测量。系统
  仅在 UE--track association 已通过 cost/margin acceptance 且对应 track 当前可观测时使用小候选集，否则
  conventional fallback，并以 candidate recall、CSI-RS overhead、sensing use/fallback 和 average effective user
...
- Chapter 4 介绍分布式多模态 BEV perception、joint vehicle detection and beam prediction、IMM-based
  tracking、target-to-user association、association-gated candidate selection 与通信接口。
- Chapter 5 说明多模态 V2I 数据如何构成评测样本，定义冻结协议和指标，并依次报告多模态感知、
  association-gated 波束管理与跟踪结果；所有比较在一致的 measurement/resource profile 下进行。
...
本章目的：基于既有多模态数据资产构造面向目标 BS 的 V2I 评测样本和 beam-management episodes，并在统一的 5G NR-oriented measurement/resource profile 下，报告从目标检测与波束预测到 association-gated 候选测量、再到 average effective user rate 的完整结果链。重点是复用既有感知和链路资产进行一致的系统评价，而非重新生成原始图像、点云或无线信道。

叙事逻辑：先界定多模态 V2I 数据如何组织为可复现的训练、验证和测试样本；随后定义只服务于正式结果的指标与冻结协议；再依次报告多模态/分布式感知、association-gated 波束管理和跟踪模块的结果。这样的顺序将上游的可观测性和候选质量与下游的测量开销和有效吞吐率直接连接，同时避免把单一 Top-K 指标误作系统收益。
...
- 给出冻结 radio profile：100 ms SSB periodicity、20 ms CSI-RS periodicity、每个 SSB parent 的四波束 conventional refinement、一致的 RT segment、Doppler、功率、噪声、RZF 和归一化资源记账。雨雾条件使用 min_association_margin=0.10 的固定 test-tuned operation point，应表述为本结果包的运行配置，而非独立的 held-out 泛化结论。
...
- 以 camera-only、ISAC-only、single-station multimodal、node-mean fusion 和 cross-agent multimodal 作为同一任务上的已报告方案，在本节描述其输入/融合差异，不单设 Baselines 一节。
...
- 以 Table 5.1 总体比较五类感知/融合方案；按天气的结果拆分为 Table 5.2 的 detection metrics 和 Table 5.3 的 beam-prediction metrics，以避免将不同层次指标压缩进过宽表格。叙述应指出：cross-agent multimodal 的 AP@2m、Recall@2m 和 Top-4 power ratio 分别为 78.73%、81.51% 和 97.01%，而 single-station multimodal 的 Beam Top-1@2m 和 Beam Top-4@2m 分别为 71.54% 和 94.66%；不同方案在 target availability 与 beam ranking 上仍体现不同侧重。
...
### 5.4 Results: Association-Gated Beam Management
...
- 将 54.35% sensing use 与 45.65% fallback 解释为 association-gated policy 的混合运行状态。公共的 data-fraction 项来自冻结的数据侧资源配置，因此不应被解读为另一项由 policy 单独优化的结果。
...
#### 5.5.1 End-to-End Tracker Comparison
