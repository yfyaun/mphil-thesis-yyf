...
- 使用 Table 5.4 对比三种天气下 DMSA-BM 与 SSB-guided CSI-RS refinement 的两个 operating points。表格仅保留 effective user rate 与 CSI-RS overhead，并在末尾增加三天气等权 Macro average，删除与后续比较重复的 sensing-use 列。
- clear_day 中 DMSA-BM 的平均 rate 为 155.453 Mbps、CSI-RS overhead 为 1.39%；对应的 \(K_{\mathrm{ref}}=4\) 和 \(K_{\mathrm{ref}}=12\) refinement 分别达到 151.367/1.90% 与 127.033/5.71%。Macro average 下三者分别为 146.332/1.56%、142.248/1.90% 与 115.890/5.71%。
- 用正文解释 DMSA-BM 在 Rain/Fog 和 Night 下更多回退至 conventional refinement，但不在该表重复展示 sensing use；强调 per-weather 数字是固定 operating point 下的 policy consequence。

#### 5.4.2 Impact of Sensing Configuration

- 使用 Table 5.5 比较 Camera-only、ISAC-only、Local camera--ISAC fusion 与完整 DMSA-BM 的端到端通信结果，只报告三天气等权平均的 effective user rate、CSI-RS overhead 与 sensing use。
- DMSA-BM、Local camera--ISAC fusion、ISAC-only 与 Camera-only 的 rate 分别为 146.332、143.701、142.522 与 138.868 Mbps；CSI-RS overhead 分别为 1.56%、1.80%、1.81% 与 1.86%；sensing use 分别为 54.35%、58.28%、49.03% 与 47.93%。
- 解释 Local camera--ISAC fusion 是三个简化输入方案中最接近完整方案的配置，说明本地模态互补已带来主要收益，而分布式视角进一步改善端到端结果。DMSA-BM 的 sensing use 并非最高，却同时具有最高 rate 和最低 overhead，说明通信收益取决于可用候选的质量及其关联有效性，而非 sensing-use 频率本身。
- 明确该表比较完整端到端配置：三个简化方案来自保存的 modality-ablation runs，DMSA-BM 采用正式 selected operating point；因此不将差异全部归因于单一网络层。
- 原 Figure 5.4 与 Figure 5.5 删除，避免与两张表重复呈现 rate、overhead 和 sensing-use 信息。
