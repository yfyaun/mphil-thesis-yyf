...
- 说明 Top-1 classifier 为全部 192 个 CSI beams 导出 scores，而 \(K_{\mathrm{scan}}\) 才是实际 CSI-RS
  scan budget；不得将 classifier output length 误写为 pilot budget。
...
- 将 multimodal observation 映射为未绑定 detections 与每个 target 的 192-way CSI beam-class posterior；
  detection 不含 UE identity，posterior 由 Top-1 CE classifier 导出并用于候选排序。
- 将 sensing-level objective 表述为 target detection quality 与条件 Top-1/Top-\(K\) beam-prediction
  performance；Top-1 CE loss 和 joint training 留到 Chapter 4。
...
- 对 initial/re-association event，使用 measured CSI beam 与 track beam-posterior score 构造 cost；SSB-parent
...
- 用 rate--overhead evaluation vector 而非单一标量总结系统目标：detection/beam ranking、accepted
  association 的运行证据、candidate recall、CSI-RS overhead、all-UE effective rate 与 handover
  diagnostics。章节结尾
...
- 以 Table 5.3 和 Figures 5.1--5.3 报告在固定模型下可用 nearby nodes 数从 0 到 5 的变化，明确这些是 missing-node robustness/coverage curves，不是为每个节点数量分别重训练的容量比较。
...
- 使用 Table 5.5 和 Figure 5.5 汇总宏平均：perception-assisted policy 的 effective user rate 为 146.332，CSI-RS overhead 为 1.56%，而 conventional four-beam 与 twelve-beam 分别为 142.248/1.90% 和 115.890/5.71%。
