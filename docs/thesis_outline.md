...
measurement 与用户速率之间的物理联系，不把 perception、tracking、association、candidate selection
和 scheduling 强行写成一个单一联合优化问题。
...
rate。BEV fusion、network loss、Hungarian association、IMM recursion 和 candidate reliability rule 留给
Chapter 4。
...
- 先以一般单位范数 precoding vector 定义 BS transmit signal，不在此处说明 beam selection 或 precoder 的
  获取方式。对 \(\mathcal U_b^{\mathrm{sch}}[n]\) 给出多用户接收信号；目标流、multi-user interference
  和噪声共同定义 downlink SINR。
- 以 scheduling indicator、bandwidth 和 downlink resource fraction 定义尚未扣除 beam-management overhead
  的 gross downlink achievable rate，为 3.3.3 的 effective rate 建立对照。
...
- 由 CSI-RS power、equivalent-channel estimate、interference 和 noise 定义 per-beam measured SINR/quality；
  ...
  quality information；multi-user RZF 所需的 scheduled RF-beam coefficients 以相同 beam-domain CSI 形式反馈。
...
- 汇总 scheduled VUE 的 beam-conditioned feedback CSI，并给出 RZF precoder 与 post-RZF SINR。RZF 是
...
- 以语言说明 conventional flow：先扫 48-beam SSB codebook 得到 coarse direction，再在对应 refined
  ...
  sensing-assisted policy 只在 association 和当前 evidence 可靠时缩小 CSI-RS set，否则 conventional fallback；
...
- 以 data resource fraction 乘 gross downlink rate 定义 \(R_{b,u,n}^{\mathrm{eff}}\)。未调度的 active VUE
  rate 计为零；all-UE time average 留作 Chapter 5 的评价统计量，而不是在本章写成跨模块联合优化问题。
