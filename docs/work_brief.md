...
4. 在计入 SSB、CSI-RS、CSI aging、RZF 和 fallback 后，基于 association acceptance 的感知辅助能否形成可验证的 rate--overhead trade-off？
...
- **H4：关联接受后使用感知。** association cost 与 assignment margin 决定是否接受 UE--track binding，current-observation requirement 进一步决定已接受关联能否产生 sensing candidates；任一条件不满足时均使用 SSB-guided refinement。
...
3. 建立 multimodal V2I simulation and 5G NR-oriented beam-management evaluation framework，将多模态感知、时变 V2I 信道、communication measurement 与资源记账置于一致的评价链条中。该框架以 average effective user rate、CSI-RS overhead 和 sensing-use/fallback 刻画 perception-assisted policy 的通信后果，从而区分 beam-prediction accuracy 与实际系统收益。

## 3. 系统模型、算法接口与符号约定

### 3.1 多模态局部观测

对 sensing epoch \(n\) 的 target BS \(b\)，多模态观测记为

\[
\mathcal{O}_{b,n}=\{\mathcal{I}_{b,n},\mathcal{I}_{\mathcal{N}_b,n},\mathcal{P}_{b,n}\},
\]

其中，\(\mathcal{I}_{b,n}\) 是 target BS 的多视角相机观测，\(\mathcal{I}_{\mathcal{N}_b,n}\) 是邻近 SN 的相机观测，\(\mathcal{P}_{b,n}\) 是 BS-local ISAC 点云。相机特征经标定投影至 BEV，点云特征在同一网格编码，随后完成跨节点和跨模态融合。

BS-local ISAC 点云在 Chapter 3 中采用 monostatic radio-sensing abstraction 解释。BS 以已知 sensing symbol 和 sensing beam 发射探测信号，接收回波信道分解为 stationary roadside structures 形成的 static component 与 moving objects 形成的 dynamic component。dynamic component 进一步包含目标直接往返回波，以及目标参与、并附加一次环境镜面或漫反射的 secondary paths；其 delay、angle 和 Doppler 随目标运动变化。static component 可通过背景估计或 slow-time averaging 获取，并由常见 clutter suppression 去除。对剩余 target-related channel 进行 range--angle--Doppler processing，再以 CFAR 检测保留显著 resolution cells，最终将 range/angle 映射为 BS-local point cloud \(\mathcal P_{b,n}\)。该建模只阐明已有 ISAC 点云的物理来源，不将新 waveform、clutter-removal 或 CFAR 算法作为本文贡献。

设 \(\mathcal{J}_b=\{b\}\cup\mathcal{N}_b\)，相机特征先按节点组织并在 target-BS-centred BEV 中对齐。对 BEV cell \(\mathbf x\)，令 \(\mathbf f^{\mathrm{cam}}_{j\rightarrow b,n}(\mathbf x)\) 表示节点 \(j\) 的特征，\(m^{\mathrm{node}}_{j,n}(\mathbf x)\in\{0,1\}\) 表示该节点在该位置是否具有有效投影。正式方法将节点聚合表述为 **attention-based cross-node fusion**，其中 cross-node fusion 是模块功能，multi-head attention 是实现机制。以 target-BS feature 与有效节点均值共同构造 query，以各节点 feature 和相对几何编码构造 key/value。第 \(h\) 个 head 的权重可概括为

\[
e_{j,h,n}(\mathbf x)=
\frac{\mathbf q_{h,n}(\mathbf x)^{\mathsf T}\mathbf k_{j,h,n}(\mathbf x)}{\sqrt{d_h}}
+b_h(\mathbf g_{j\rightarrow b}),\qquad
\alpha_{j,h,n}(\mathbf x)=
\frac{m^{\mathrm{node}}_{j,n}(\mathbf x)\exp(e_{j,h,n}(\mathbf x)/T_{\mathrm{node}})}
{\sum_{\ell\in\mathcal J_b}m^{\mathrm{node}}_{\ell,n}(\mathbf x)\exp(e_{\ell,h,n}(\mathbf x)/T_{\mathrm{node}})+\epsilon}.
\]

各 head 以 \(\sum_j\alpha_{j,h,n}\mathbf v_{j,h,n}\) 聚合不同节点，并通过 output projection、masked-mean residual 与 FFN refinement 得到 \(\mathbf F^{\mathrm{cam}}_{b,n}\)。该设计允许不同 BEV 位置依据目标 BS 上下文、节点相对位置和局部可见性选择互补视角，而不是对所有 SN 固定平均。

BS-local ISAC point cloud 被编码为 \(\mathbf F^{\mathrm{isac}}_{b,n}=\Phi_{\mathrm{pc}}(\mathcal P_{b,n})\)。正式方法进一步采用 **gated multimodal attention**。首先将 camera 与 ISAC features 投影到相同通道数，并构造

\[
\mathbf U_{b,n}=\left[
\widetilde{\mathbf F}^{\mathrm{cam}}_{b,n}\,\|\,
\widetilde{\mathbf F}^{\mathrm{isac}}_{b,n}\,\|\,
\left|\widetilde{\mathbf F}^{\mathrm{cam}}_{b,n}-
\widetilde{\mathbf F}^{\mathrm{isac}}_{b,n}\right|
\right].
\]

对模态 \(q\in\{\mathrm{cam},\mathrm{isac}\}\)，local gate 产生逐 cell logit \(\ell^{\mathrm{loc}}_{q,b,n}(\mathbf x)\)，global gate 从 \(\operatorname{GAP}(\mathbf U_{b,n})\) 产生 scene-level logit \(\ell^{\mathrm{glo}}_{q,b,n}\)。令 \(a_{q,b,n}\in\{0,1\}\) 为 modality availability mask，则

\[
\beta_{q,b,n}(\mathbf x)=
\frac{a_{q,b,n}\exp\!\left((\ell^{\mathrm{loc}}_{q,b,n}(\mathbf x)+\ell^{\mathrm{glo}}_{q,b,n})/T_{\mathrm{modal}}\right)}
{\sum_{q'}a_{q',b,n}\exp\!\left((\ell^{\mathrm{loc}}_{q',b,n}(\mathbf x)+\ell^{\mathrm{glo}}_{q',b,n})/T_{\mathrm{modal}}\right)+\epsilon},
\]

\[
\mathbf F^{\mathrm{modal},0}_{b,n}(\mathbf x)=
\sum_q\beta_{q,b,n}(\mathbf x)\widetilde{\mathbf F}^{q}_{b,n}(\mathbf x),\qquad
\mathbf F_{b,n}=\sigma\!\left(\mathbf F^{\mathrm{modal},0}_{b,n}+\Phi_{\mathrm{ref}}(\mathbf F^{\mathrm{modal},0}_{b,n})\right).
\]

local attention 处理遮挡、点云稀疏性与空间位置相关的模态质量差异，global attention 响应整幅场景中的天气、照明和整体观测条件；权重通过 detection 与 beam-prediction objectives 端到端学习。\(m^{\mathrm{node}}\) 与 \(a_q\) 仅表示投影有效性或模态是否存在，不能解释为独立 reliability model。
...
若 \(\widehat{\mathbf r}_{b,i,n}\) 是检测中心、\(\widehat{\mathbf a}_{b,i,n}\) 是几何或运动属性、\(\widehat c_{b,i,n}\) 是检测置信度，则 target-level 输出写为

\[
d_{b,i,n}=(\widehat{\mathbf r}_{b,i,n},\widehat{\mathbf a}_{b,i,n},\widehat c_{b,i,n}),
\qquad
\mathbf p_{b,i,n}=\operatorname{softmax}\!\left(
\operatorname{Sample}(\mathbf P_{b,n},\widehat{\mathbf r}_{b,i,n})\right),
\]

其中 \(\mathbf P_{b,n}\) 是 dense 192-way beam-class logit map，\(\mathbf p_{b,i,n}\in[0,1]^{192}\) 是未绑定 physical target 的 CSI beam-class posterior。其 Top-\(K\) entries 作为后续候选测量和关联的 target-level score evidence，而非 UE-specific service-beam declaration 或物理功率分布估计。
...
\[
\mathbf{x}^{\mathrm{CV}}_{r,n}=[x_{r,n},y_{r,n},v_{x,r,n},v_{y,r,n}]^{\mathsf T},\qquad
\mathbf{z}_{i,n}=[\widehat{x}_{b,i,n},\widehat{y}_{b,i,n}]^{\mathsf T}.
\]
...
tracking 在本文中用于维持 beam hint 所属感知对象的时间连续性，而不是声称某一滤波器本身构成主要创新。匹配检测更新 track motion state、mode probability、confidence、observation history 和当前 beam-class posterior；未匹配轨迹可短时保留，但纯预测状态不生成正式 beam candidate。GT/noisy-GT 结果仅用于运动模型和数据关联的受控诊断，不能替代 predicted-detection 的端到端主结果。

### 3.4 测量辅助 UE--track binding

感知轨迹没有通信身份。在初始接入、切换到新 serving cell 或已有 binding 失去支持时，系统执行 initial association 或 reassociation。当前帧未观测只会阻止 sensing-derived candidate reduction，并触发 SSB-guided refinement，不自动等同于一次新的 association event。令 conventional measurement 为 communication VUE \(u\) 选出的 CSI-RS beam 为 \(\kappa_{b,u,n}\)，当前 observed perception track \(r\) 的 beam-class posterior 为 \(\mathbf p^{\mathrm{obs}}_{r,n}\)，则关联代价为

\[
c^{\mathrm{assoc}}_{u,r,n}
=-\frac{\log\!\left(p^{\mathrm{obs}}_{r,n}(\kappa_{b,u,n})+\epsilon\right)}{\log M_{\mathrm{CSI}}}.
\]

对 Hungarian assignment 选出的真实配对 \((u,r)\)，令 \(\mathcal R^{\mathrm{alt}}_{u,r,n}\) 包含同一 VUE 的其他可行 track 以及 unmatched 选项，则

\[
\Delta_{u,r,n}=
\min_{r'\in\mathcal R^{\mathrm{alt}}_{u,r,n}}
c^{\mathrm{assoc}}_{u,r',n}-c^{\mathrm{assoc}}_{u,r,n},
\qquad
A_{u,r,n}=\mathbf 1\!\left\{
c^{\mathrm{assoc}}_{u,r,n}\leq\tau_{\mathrm{assoc}},\quad
\Delta_{u,r,n}\geq\tau_{\mathrm{margin}}
\right\}.
\]

只有当 \(\varphi_b(\kappa_{b,u,n})=s_{b,u,n}\) 且 track \(r\) 当前可观测时，该 UE--track 配对才可行。系统在可行配对和未匹配虚拟列上执行 Hungarian assignment；高代价或低 margin 的配对被拒绝，并继续沿用 conventional scanning。该规则不使用 ground truth、不以预测位置反推码本的几何近似，也不以人工 UE ID 作为模型输入。

`beam_direction` 和 `beam_topk` 可作为 association ablation；正式叙事以 beam-posterior score、SSB-parent consistency、association cost threshold 与 assignment margin 为主。当前正式结果没有单独报告 binding coverage 或 accepted-association accuracy，因此正文只通过 sensing-use/fallback 说明 association-to-policy 的运行后果。

### 3.5 关联接受门控、候选扫描与回退

已建立 binding 后，系统保留 \(G_{u,n}\) 作为 candidate-use indicator，但不再为其引入独立的 reliability feature vector 或 threshold function。令 \(A_{u,r,n}\) 表示由 association cost threshold 与 assignment margin 决定的接受状态，则

\[
G_{u,n}=
\begin{cases}
A_{u,r,n}, & \pi_{b,n}(u)=r\in\mathcal R_{b,n}^{\mathrm{obs}},\\
0, & \text{otherwise}.
\end{cases}
\]

accepted status 在两次 association event 之间随 binding 保持，并在 association 失效时置零。仅当已接受的 binding 对应当前 observed track 时使用感知候选；否则进行 SSB-guided refinement。association confidence、detection/track confidence、observation count 和 beam-score mass 不作为第二层 candidate-use gate 的输入。

若当前 observed detection 的前 \(K_{\max}\) 个 beam scores 为 \(s_1\geq\cdots\geq s_{K_{\max}}\)，自适应候选数可定义为

\[
K^{\star}_{\mathrm{scan}}=\min\left\{k\in\{1,\ldots,K_{\max}\}\ \middle|\
...
CSI-RS candidate width；否则使用 SSB-guided refinement。网络导出的 192-way posterior 不等于 192 次导频测量，实际测量开销仅由每个 event 的 \(K_e\) 决定。
...
其中 \(K_e\) 是 acquisition event \(e\) 实际测量的 CSI-RS beam 数，\(M\) 是一个 acquisition occasion 可复用的 beam measurement 数，\(\delta_{\mathrm{probe}}\) 是一次 acquisition occasion 的归一化成本。令 \(\gamma_{b,u,n}^{\mathrm{RZF}}\) 表示真实 CSI-RS measurement、信道估计与 RZF 后的 SINR，则有效速率指标为
...
\[
\bar R_{\mathrm{eff}}=\frac{1}{N}\sum_{n=1}^{N}
\frac{1}{|\mathcal U_b[n]|}\sum_{u\in\mathcal U_b[n]}R_{b,u,n}^{\mathrm{eff}}
\]
定义 BS \(b\) 的 average effective user rate，并将其作为 Chapter 5 的系统级评价统计量。Chapter 3 不再把 sensing、association、candidate selection 与 precoding 强行写成一个联合优化问题；这些模块分别在 Chapter 4 给出算法，其共同通信后果通过 post-RZF SINR、CSI-RS overhead 与 \(R_{b,u,n}^{\mathrm{eff}}\) 体现。正式结果报告 average effective user rate、CSI-RS overhead 与 sensing use/fallback；当前正式包不将 candidate count、p05 rate 或 outage 作为正文主结果。
...
3. **per-BS 多目标轨迹维护：** 采用 CV、CT 或 IMM 在相同 BS、天气和 split 的序列内预测轨迹；通过 gated Hungarian assignment 将当前 detections 与预测轨迹关联，更新 track state、mode probability、confidence、observation history 和当前 beam-class posterior。
...
5. **关联接受门控候选 CSI-RS：** 若 UE--track association 已通过 \(A_{u,r,n}\) 接受且对应 track 当前可观测，则从当前 observation 的 beam-class posterior 形成固定或自适应 \(K_{\mathrm{scan}}\) 候选；否则使用 SSB-guided refinement。
6. **实测选择、RZF 与资源评价：** 在真正扫描的 CSI-RS candidates 内根据实测 SNR 选择 beam；在一致的 RT/Doppler、RZF、功率和归一化 \(\tau\) accounting 条件下计算 CSI-RS overhead、sensing use/fallback 与 average effective user rate。
...
| 多目标 tracking | 同一 `(split, weather_id, bs_id)` 序列的未绑定 detections 与既有 track state | perception tracks、状态预测、track confidence、observation history 与当前 beam-class posterior | track 只维护感知对象身份；纯预测位置不产生正式 CSI-RS candidate |
| UE--track binding | 当前 observed tracks、传统 SSB/CSI-RS 测量、SSB-parent consistency | association cost、assignment margin、接受或拒绝的 partial binding | ground truth 仅用于离线正确性统计，不能参与匹配代价或接受决策 |
| 关联接受门控与候选扫描 | association acceptance \(A_{u,r,n}\)、当前 observed track 及其 beam-class posterior | fixed/adaptive \(K_{\mathrm{scan}}\) candidates 或 SSB-guided refinement | 不引入第二层 reliability features；prediction 只缩小实测集合，最终服务 beam 仍由实际 CSI-RS/SNR 测量决定 |
| PHY 与资源评价 | 实测候选、统一的 RT/Doppler、RZF、功率和归一化 \(\tau\) profile | CSI-RS overhead、sensing use/fallback 与 average effective user rate | 不从通信结果反向修改 test 阈值或把 oracle ceiling 作为正式方案 |
...
| Tracking | 不同运动模型如何影响轨迹维护与端到端 policy | mean effective user rate、sensing use，以及受控轨迹诊断 | 不能仅由受控轨迹图证明端到端身份优势 |
| Candidate selection and system | 更少 beam 的候选测量是否在资源代价下仍有收益 | mean effective user rate、CSI-RS overhead、sensing use、fallback | 不能反推任一感知模块在所有条件下最优 |
...
| Tracker 宏平均比较 | Top-1 CE detector 下，IMM、KF-CV 和 KF-CT 的 mean effective user rate 分别为 146.332、146.373 和 145.880 Mbps，sensing use 分别为 54.35\%、53.80\% 和 52.26\% | KF-CV 在该固定操作点的 rate 略高于 IMM，但三种 tracker 的端到端差异有限，不能据此宣称跨运动条件或系统配置的普遍最优 | 冻结正式结果包，`table_06_tracker_macro_average` |
...
- 不得宣称 IMM 在 predicted-detection 端到端路径上普遍优于 CV/CT；正式宏平均表中 KF-CV 的 user rate 略高于 IMM，且只报告 mean effective user rate 与 sensing use，解释应保持在该证据范围内。
...
| 关联接受门控 | association-acceptance gate | 由 \(A_{u,r,n}\) 与 current-observation requirement 直接决定是否使用 sensing hint，不包含第二层 reliability features |
